...
@Component({
  selector: 'app-brickwork-stage',
  templateUrl: './brickwork-stage.component.html',
  styleUrls: ['./brickwork-stage.component.css'],
})
    ...
    this.projectService.getProjectById(this.id).subscribe((project: any) => {
      ...
      this.stageService.getStagesByProject(this.id).subscribe((stages) => {
        ...
        this.stageList.forEach((element) => {
          if (
            element.stageName ===
            'Brick Work - ' + this.project.projectName
          ) {
            this.element = element;
            this.brickworkForm = new FormGroup({
  ...
  onImages() {
    ...
    this.imageService
      .getImages(
        ...
        this.brickworkForm.value.elementid
      )
